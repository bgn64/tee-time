 ...
 * Run via:
 *
 *   tsx scripts/ingest-opengolf.ts                          # uses .env.local
 *   tsx scripts/ingest-opengolf.ts --dry-run                # parse + stats, no DB writes
 *   tsx scripts/ingest-opengolf.ts --env .staging.env       # load env vars from .staging.env
 *   tsx scripts/ingest-opengolf.ts --env .prod.env          # load env vars from .prod.env
 *
 * Required env (loaded from the env file specified by `--env`, or
...
if (!dryRun && (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY)) {
  console.error(
    `Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY. Add them to ${envFile} before running (or pass --dry-run).`
  );
  process.exit(1);
}

if (!dryRun) {
  // Print a non-secret breadcrumb so an operator running this can
  // confirm at a glance which environment they're about to write to.
  console.log(`[ingest] Using env file: ${envFile}`);
  console.log(`[ingest] Target SUPABASE_URL: ${SUPABASE_URL}`);
}
