/**
 * Custom-score sheet — opened from the ✕ chip on ScoreEntryRow when the
 * user needs a value outside the −2…+2 quick-pick range. Shows a large
 * relative-to-par display with − / + steppers (any integer relative
 * value supported, clamped to a minimum of 1 stroke), the implied raw
 * stroke count, and a Done button to commit.
 *
 * Opens with `initialStrokes` (or par when null) so the user can adjust
 * directly. Cancel discards.
 */
...
type Props = {
  ...
  holeNumber: number;
  par: number;
  /** Current value, or null when the score is unset. */
...
export function CustomScoreSheet({
  ...
  // Local draft so the user can adjust without committing until Done.
  // Default to one above par (the most common reason to reach this sheet
  // is a high score; the user's first tap of + or − adjusts from there).
